...
class TransactionsAdapter(
    ...
    inner class TransactionViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        private fun formatAmount(amount: Double, type: TransactionType): String {
            val currencyFormatter = NumberFormat.getNumberInstance(Locale.getDefault())
            val formattedAmount = currencyFormatter.format(amount)
            val preferenceManager = PreferenceManager(itemView.context)
            val prefix = if (type == TransactionType.INCOME) "+${preferenceManager.getCurrency()}" else "-${preferenceManager.getCurrency()}"
